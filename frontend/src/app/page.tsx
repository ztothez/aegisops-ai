...
export default function SOCCommandCenter() {
  ...
  const runPipeline = async (overrideMode?: string, overrideTarget?: string) => {
    ...
    setResults(null); setActiveAgent(null); setAgentTimes({});
    agentStartRef.current = {}; collectedOutputs.current = {};

    try {
      const res = await fetch(`${API}/run`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ technique_id: tid, demo: demoMode, mode: currentMode.toLowerCase().replace(/ /g, "_") }),
      });
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          if (ev === "agent_start") {
            ...
            const outputs = {
              red: collectedOutputs.current["red"] ?? "", blue: collectedOutputs.current["blue"] ?? "",
              response: collectedOutputs.current["response"] ?? "", verifier: collectedOutputs.current["verifier"] ?? "",
            };
            const run: RunResult = { ...(data as any), outputs, technique_id: tid, mode: currentMode, timestamp: Date.now() };
            setResults(run);
            setRunHistory(h => [run, ...h.slice(0, 19)]);
            setLogs(p => [...p, `${ts()} exports ready · Sigma · SPL · VECTR · JSON`, `${ts()} VALIDATED · ${run.scores.safety_verdict} · coverage=${run.scores.coverage}%`]);
            setStatus("success"); setActiveAgent(null);
          }
        }
      }
    } catch (e: any) {
      setStatus("error"); setLogs(p => [...p, `${ts()} CRITICAL: ${e.message}`]);
    }
  };
  ...
  const heroPillClass = endpointOk ? "bg-aegis-tint-green border-aegis-border-green text-aegis-green" : "bg-aegis-tint-amber border-aegis-border-amber text-aegis-amber";
  ...
  const ArtifactGridComponent = ({ run }: { run: RunResult }) => {
    ...
    const canonicalId = isGroup ? (run.technique_id.includes("APT") ? run.technique_id : "APT_Run") : run.technique_id;
          ...
          <div key={i} className={`bg-aegis-panel-3 border border-aegis-border-hi border-t-2 ${art.top} rounded-lg px-[14px] py-3 h-[120px] flex flex-col justify-between`}>
            ...
            <button onClick={async () => {
              if (art.label === "PDF REPORT") {
                ...
                a.click();
              } else dl(art.filename, typeof art.content === 'string' ? art.content : JSON.stringify(art.content));
                  ...
                  <div className="px-4 pb-4 pt-2 border-t border-aegis-border-hi bg-aegis-panel">
                    <div className="grid grid-cols-2 gap-4 mt-2">
                      <div>
                        ...
                        <div className="bg-aegis-terminal border border-aegis-border-hi rounded-md p-3 h-80 overflow-y-auto font-mono text-[11.5px] leading-relaxed text-slate-300 whitespace-pre-wrap custom-scrollbar">
                  ...
                  <div className="px-4 pb-4 pt-2 border-t border-aegis-border-hi bg-aegis-panel-3">
                    <div className="grid grid-cols-2 gap-4 mt-2">
                      <div>
                        ...
                        <div className="bg-aegis-terminal border border-aegis-border-hi rounded-md p-3 h-64 overflow-y-auto font-mono text-[11.5px] leading-relaxed text-slate-300 whitespace-pre-wrap custom-scrollbar">
  ...
  const renderThreats = () => {
    ...
    const tacticColors: Record<string, string> = { "Execution": "#EF4444", "Initial Access": "#F59E0B", "Credential Access": "#3B82F6", "Lateral Movement": "#22C55E", "Defense Evasion": "#C4B5FD" };
    ...
    return (
      <div className="flex-1 px-[22px] py-[18px] pb-8 overflow-y-auto">
        ...
        <input value={threatSearch} onChange={e => setThreatSearch(e.target.value)} placeholder="Search technique ID, name, or tactic…" className="w-full bg-aegis-input border border-aegis-border-hi rounded-md px-3.5 py-2.5 text-aegis-fg font-mono text-[13px] outline-none mb-[18px]" />
        <div className="grid grid-cols-3 gap-2.5">
          {catalog.map((t, i) => (
            <div key={i} onClick={() => { setTechnique(t.id); setMode("Single Technique"); setActiveView("overview"); }} className="bg-aegis-panel-3 border border-aegis-border-hi border-t-2 rounded-lg px-3.5 py-3 cursor-pointer hover:bg-aegis-panel-2 transition-colors" style={{ borderTopColor: tacticColors[t.tactic] || "#475569" }}>
  ...
  const renderHunting = () => {
    // Topolgy Lab Data mapped over from the original Hackathon UI
    const ATTACK_PATHS = [
      { id: "phish", label: "Phishing to PowerShell to C2", summary: "User execution leads to PowerShell, persistence, and C2 telemetry.", seed_techniques: ["T1566.001", "T1204.002", "T1059.001"], hops: [
      ...
      { id: "valid_acct", label: "Valid Account to Domain Credential Access", summary: "Compromised credentials enable remote access and credential dumping.", seed_techniques: ["T1078", "T1021.001", "T1003.001"], hops: [
    ...
    const activeNodes = new Set(path.hops.flatMap(h => [h.from, h.to]));
        ...
        <div className="flex gap-3 mb-4 items-end">
          ...
          <div className="flex-[2]">
            ...
            <select value={selectedPath} onChange={e => setSelectedPath(Number(e.target.value))} className="w-full bg-aegis-input border border-aegis-border-hi rounded-md px-3.5 py-2.5 text-aegis-fg font-mono text-[13px] outline-none">
              {filteredPaths.map((p, i) => <option key={i} value={i}>{p.label}</option>)}
            </select>
          </div>
        </div>
  ...
  const renderIntel = () => (
    <div className="flex-1 px-[22px] py-[18px] pb-8 overflow-y-auto">
      ...
      <div className="flex gap-2.5 mb-[18px]">
        <input value={intelInput} onChange={e => setIntelInput(e.target.value)} placeholder="APT Group name, e.g. APT28" className="flex-1 bg-aegis-input border border-aegis-border-hi rounded-md px-3.5 py-2.5 text-aegis-fg font-mono text-[13px] outline-none" />
        <button onClick={() => { 
          setMode("APT Group"); 
  ...
  const renderReports = () => {
    ...
    const canonicalId = isGroup ? (latestRun.technique_id.includes("APT") ? latestRun.technique_id : "APT_Run") : latestRun.technique_id;
      ...
      <div className="flex-1 px-[22px] py-[18px] pb-8 overflow-y-auto">
        ...
        <div className="bg-gradient-to-br from-aegis-panel-3 to-aegis-panel border border-aegis-border-purple rounded-xl p-6 mb-6 shadow-aegis-card relative overflow-hidden">
          ...
          <div className="flex justify-between items-start">
            ...
            <button onClick={async () => {
              const r = await fetch(`${API}/export/pdf`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ technique_id: canonicalId, red_output: latestRun.outputs.red, blue_output: latestRun.outputs.blue }) });
              const blob = await r.blob();
              const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `aegisops_${canonicalId}.pdf`; a.click();
            }} className="mt-2 px-6 py-3 bg-gradient-to-br from-[#7C3AED] to-[#4F46E5] text-white rounded-lg font-sans text-xs font-bold uppercase tracking-[0.08em] shadow-aegis-hover hover:scale-[1.02] transition-transform">
              ↓ Download PDF
            </button>
          </div>
        </div>
        ...
        {runHistory.length > 1 && (
          <div>
            ...
            {runHistory.slice(1).map((run, i) => {
              const cid = run.mode.toLowerCase() === "apt group" ? (run.technique_id.includes("APT") ? run.technique_id : "APT_Run") : run.technique_id;
              return (
                <div key={i} className="bg-aegis-panel-2 border border-aegis-border-hi rounded-md px-4 py-3 mb-2 flex justify-between items-center hover:bg-aegis-panel transition-colors">
                  ...
                  <button onClick={async () => {
                    const r = await fetch(`${API}/export/pdf`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ technique_id: cid, red_output: run.outputs.red, blue_output: run.outputs.blue }) });
                    const blob = await r.blob();
                    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `aegisops_${cid}.pdf`; a.click();
      ...
      <div className="grid grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] gap-[18px]">
        <div className="bg-gradient-to-b from-aegis-panel-2 to-aegis-panel border border-aegis-border-grid rounded-lg px-5 py-[18px]">
          ...
          <div className="bg-aegis-bg-deep border border-aegis-border-hi rounded-md px-3.5 py-2.5 flex items-center gap-2 mb-3">
            ...
            <input 
              value={technique} onChange={e => setTechnique(e.target.value)} onKeyDown={e => { if (e.key === "Enter") runPipeline(); }} disabled={status === "running"}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <label className="flex items-center gap-2 font-sans font-medium text-[11px] text-aegis-fg-dim cursor-pointer select-none">
              <div onClick={() => setDemoMode(d => !d)} className={`w-[36px] h-[20px] rounded-full relative cursor-pointer transition-colors duration-200 ${demoMode ? 'bg-aegis-purple/50' : 'bg-aegis-border-hi'}`}>
            ...
            {logs.map((line, i) => {
              ...
              return (
                <div key={i} className="font-mono font-normal text-[11.5px] leading-[1.5]">
                  ...
                  <span className={`
                    ${line.includes("VALIDATED") ? "text-aegis-green-soft font-bold" : 
                      line.includes("ERROR") || line.includes("CRITICAL") ? "text-aegis-red-soft" : 
                      line.includes("ready") ? "text-aegis-purple-soft" : "text-aegis-fg-muted"}
                  `}>{rest}</span>
                </div>
              );
            })}
